...
def optimize(berat, profit, populasi, ukuran_populasi, no_generasi, max_tas):
    ...
    for i in range(no_generasi):
        ...
        while (parent.shape[0] < no_offspring):
            ...
            i=+1
        #akhir code crossover

        #code untuk proses mutasi
        ...
        #tambah data ftines kedalam catatan
...
plt.plot(list(range(jumlah_gen)), fitness_history_mean, label = 'rata-rata Fitness')
plt.plot(list(range(jumlah_gen)), fitness_history_max, label = 'Fitness maksimum')
plt.legend()
plt.title('nilai fitnes setiap generasi')
plt.xlabel('Generasi')
plt.ylabel('Fitness')
plt.show()